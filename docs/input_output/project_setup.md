...
1. Setup the project directory from the pipeline repo with Cookiecutter:
    ...
    ```bash
    cookiecutter gh:utia-gc/rnaseq --directory cookiecutter --checkout v0.2.2
    ```

2. Run the project setup Nextflow script:

    ```bash
    nextflow run utia-gc/rnaseq -main-script setup.nf -revision v0.2.2 -params-file src/nextflow/setup_params.yaml
    ```
